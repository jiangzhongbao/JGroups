...
*/
        ...
        for(int i=0; i < protocol_configs.size(); i++) {
            ...
            for(int j=0; j < methods.length; j++) {
                if(isSetPropertyMethod(methods[j], protocol.getClass())) {
                    ...
                    if(propertyValue != null && InetAddressInfo.isInetAddressRelated(methods[j]))
                        visitor.accept(protocol, methods[j], properties, propertyName, propertyValue);
                }
            }
            ...
            for(int j=0; j < fields.length; j++) {
                ...
                if(propertyValue != null && InetAddressInfo.isInetAddressRelated(fields[j]))
    ...
    /*protected static void convertAddress(String input, final Collection<InetAddress> addresses) {
        ...
        if(input.indexOf(',') != -1) {
            ...
            list.stream().map(s -> s.contains("[")? s.substring(0, s.indexOf('[')) : s).forEach(a -> convertAddress(a, addresses));
            return;
        }
        ...
        try {
            ...
            if(input.startsWith("match"))
                addr=Util.getAddressByPatternMatch(input);
            else if(input.startsWith("custom:"))
                addr=Util.getAddressByCustomCode(input.substring("custom:".length()));
            if(addr == null)
